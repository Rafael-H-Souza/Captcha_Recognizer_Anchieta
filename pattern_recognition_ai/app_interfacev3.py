...
with tab3:
    ...
    if uploaded_file:
        # 1️⃣ Salva a imagem temporariamente
        temp_dir = "data/tmp"
        os.makedirs(temp_dir, exist_ok=True)
        ...
        # 2️⃣ Cria o predictor
        predictor_debug = Predictor(config["final_model_path"], debug=True)

        # 3️⃣ Chama função de predição detalhada (precisamos implementar no Predictor)
        result, debug_steps, details = predictor_debug.predict_with_debug(img_path)

        # 4️⃣ Resultado final
        st.success(f"🔍 Resultado do modelo: **{result}**")

        # 5️⃣ Mostra todas as etapas de pré-processamento
        with st.expander("🖼 Histórico de Pré-processamento"):
            for i, step in enumerate(debug_steps):
                # Se step for um dict com "step" e "image"
                ...
                st.image(img, caption=step_name, use_column_width=True)

        # 6️⃣ Mostrar tabela de detalhes
        if details:
            df_details = pd.DataFrame(details)
            for col in ["confidence", "confidence_alpha", "confidence_number"]:
                if col in df_details.columns:
